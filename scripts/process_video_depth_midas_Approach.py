...
from utils.depth_utils import get_depth_map_using_MiDas, estimate_depth_MiDas
...
def process_video_MiDas_approach(video_name, encoder='MiDaS_small'):
    """
    ...
    # Load the model 
    midas = torch.hub.load('intel-isl/MiDaS', encoder)
    midas.to(device)
    midas.eval()
    ...
    while cap.isOpened():
        ...
        if left_ankle_2d and right_ankle_2d:
            ...
            depth_map = get_depth_map_using_MiDas(frame, midas, device)

            # Estimate depth (placeholder for now)
            left_ankle_3d = estimate_depth_MiDas(depth_map, [left_ankle_2d], frame.shape)[0]
            right_ankle_3d = estimate_depth_MiDas(depth_map, [right_ankle_2d], frame.shape)[0]
    ...
    save_video(frames, f"processed_with_depth_{encoder}_{video_name}")
